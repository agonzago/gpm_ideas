...
SUBROUTINE FAST_KAL_FILT_PAT(Z,H,T,R,Q,Y,A_INI,P_INI,NUM_OBS,NUM_EST,NUM_K,NUM_DIST_EST,NUM_PER,NUM_RAT_OBS,LOG_L)
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: NUM_OBS, NUM_EST, NUM_DIST_EST, NUM_PER, NUM_K, NUM_RAT_OBS
  ...
  DO J=1, NUM_PER
     DO I=1, NUM_OBS
        ...
        ELSE ! SOLO PARA LAS OBSERVABLES QUE NO DEPENDEN DE UN ESTADO DEL FILTRO
        !zi=I 
           V= Y(I,J) 
           K= 0.0
           F= H(I)
        !RAT_SS=RAT_SS+0.5*(LOG(F)+(V**2)/dble(F))
        !RAT_SS=RAT_SS+0.5*(LOG(F)+(V**2)/dble(F))
        RAT_SS=RAT_SS+0.5*((V**2)/dble(F))
        LOG_L = LOG_L + RAT_SS
        ENDIF 
  ...
  PRINT *,"RAT_SS=",RAT_SS
  PRINT *," LOG_L=",LOG_L
END SUBROUTINE FAST_KAL_FILT_PAT
