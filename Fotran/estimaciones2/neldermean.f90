...
!   double precision y(n_max+1)
  ...
  real(dp), parameter :: TOLF = (1.0)*300.0_dp*sqrt(EPSILON(1.0_dp)) !300.0_dp*sqrt(EPSILON(1.0_dp))
  real(dp), parameter :: TOLX = (1.0)*300.0_dp*sqrt(EPSILON(1.0_dp))
...
!     write(*, '(a, i5, a, ES14.5, a,ES14.5, a, ES14.5 )'), 'icount ', icount, ' current f ', y(ilo), &
...
!   end if
  ...
  IF ( ICOUNT .LE. KCOUNT ) THEN
     ...
     crit2 = MAXVAL(PTEST)
     IF ( (crit1 .GE. TOLF )  .and. (crit2 .GE. TOLX) ) THEN 
        GO TO 50
     END IF
   ! DO JBUG=1,N+1
   !    PRINT *,"Y(",JBUG,")=",Y(JBUG)
