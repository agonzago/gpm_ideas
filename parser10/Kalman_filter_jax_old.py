...
class KalmanFilter:
    ...
    #     elif n_obs_actual > 0:
    #         # Extract the observed part of matrices
    ...
    #     else:
    #         # All observations missing - prediction only
    ...
    #         v_obs = y_obs_t - y_pred_obs    # Innovation for observed components
    ...
    def filter(self, ys: ArrayLike) -> Dict[str, jax.Array]:
        ...
        T_mat, C_full, H_full, I_s = self.T, self.C, self.H, self.I_s
        state_cov = self.state_cov
        eps = _MACHINE_EPSILON * 10
        MAX_STATE_VALUE = 1e6
        T_steps = ys_arr.shape[0]

        if T_steps == 0:
            # ... (empty handling remains the same) ...
            # Determine n_obs_actual for empty case from ys[0] if ys is not completely empty
            n_obs_actual_for_empty = 0
            if ys.shape[0] > 0 and ys.shape[1] > 0: # Check if ys itself has data to infer from
                try:
                    # Use onp here as it's outside JAX tracing for this specific empty case
                    valid_obs_idx_np_for_empty = onp.where(~onp.isnan(onp.asarray(ys[0])))[0]
                    n_obs_actual_for_empty = len(valid_obs_idx_np_for_empty)
                except IndexError:
                    pass # n_obs_actual_for_empty remains 0
            else: # ys is truly (0, N) or (N, 0)
                n_obs_actual_for_empty = 0
            ...
            return {
                ...
                'innovations': jnp.empty((0, n_obs_actual_for_empty), dtype=self.C.dtype),
                'innovation_cov': jnp.empty((0, n_obs_actual_for_empty, n_obs_actual_for_empty), dtype=self.C.dtype),
                'log_likelihood_contributions': jnp.empty((0,), dtype=I_s.dtype)
            }

        # --- PREPARE STATIC VALUES FOR NAN HANDLING ---
        # These are computed ONCE based on ys_arr[0] and treated as static thereafter.
        # It's crucial that ys_arr[0] is concrete here.
        # When filter is called from log_likelihood, ys_arr is a concrete array.
        _valid_obs_idx_py = np.where(~np.isnan(np.asarray(ys_arr[0])))[0] # Use NumPy for this one-off
        valid_obs_idx = jnp.array(_valid_obs_idx_py) # Convert to JAX array for indexing
        n_obs_actual = len(_valid_obs_idx_py)

        if n_obs_actual == self.n_obs:
            C_obs_static = C_full
            H_obs_static = H_full
            I_obs_static = jnp.eye(self.n_obs, dtype=I_s.dtype)
            # select_obs_static = lambda y_t_full_arg: y_t_full_arg # This lambda can be tricky for JAX tracing
        elif n_obs_actual > 0:
            C_obs_static = C_full[valid_obs_idx, :]
            H_obs_static = H_full[jnp.ix_(valid_obs_idx, valid_obs_idx)]
            I_obs_static = jnp.eye(n_obs_actual, dtype=I_s.dtype)
            # select_obs_static = lambda y_t_full_arg: y_t_full_arg[valid_obs_idx]
        else: # All observations missing
            C_obs_static = jnp.empty((0, self.n_state), dtype=C_full.dtype)
            H_obs_static = jnp.empty((0, 0), dtype=H_full.dtype)
            I_obs_static = jnp.empty((0, 0), dtype=I_s.dtype)
            # select_obs_static = lambda y_t_full_arg: jnp.empty((0,), dtype=y_t_full_arg.dtype)

        # --- Kalman Filter Step Function (using static observed subset) ---
        def step_static_nan(carry, y_t_full): # y_t_full is a slice from ys_arr
            x_prev_filt, P_prev_filt = carry
            x_pred_t = T_mat @ x_prev_filt
            x_pred_t = jnp.clip(x_pred_t, -MAX_STATE_VALUE, MAX_STATE_VALUE)
            P_prev_filt_sym = (P_prev_filt + P_prev_filt.T) / 2.0
            P_prev_filt_reg = P_prev_filt_sym + eps * I_s
            P_pred_t = T_mat @ P_prev_filt_reg @ T_mat.T + state_cov
            P_pred_t = (P_pred_t + P_pred_t.T) / 2.0
            P_pred_t = P_pred_t + eps * I_s

            # --- Update Step ---
            # SELECT OBSERVED PART OF y_t_full using the pre-computed valid_obs_idx
            # This direct indexing should be fine if valid_obs_idx is concrete.
            y_obs_t = jnp.take(y_t_full, valid_obs_idx, axis=0) if n_obs_actual > 0 else jnp.empty((0,), dtype=y_t_full.dtype)

            # Use C_obs_static and H_obs_static (which have fixed shapes now)
            y_pred_obs = C_obs_static @ x_pred_t
            v_obs = y_obs_t - y_pred_obs
            PCt_obs = P_pred_t @ C_obs_static.T
            S_obs = C_obs_static @ PCt_obs + H_obs_static
            S_obs_reg = S_obs + eps * 10.0 * I_obs_static # Use I_obs_static

            K = jnp.zeros((self.n_state, n_obs_actual), dtype=x_pred_t.dtype)
            if n_obs_actual > 0:
                try:
                    L_S_obs = jnp.linalg.cholesky(S_obs_reg)
                    # PCt_obs.T has shape (n_obs_actual, n_state)
                    # L_S_obs has shape (n_obs_actual, n_obs_actual)
                    # Y_obs should be (n_obs_actual, n_state)
                    Y_obs_T = jax.scipy.linalg.solve_triangular(L_S_obs, PCt_obs.T, lower=True, trans='N')
                    K = jax.scipy.linalg.solve_triangular(L_S_obs, Y_obs_T, lower=True, trans='T').T
            ...
            if n_obs_actual > 0:
                ...
                x_filt_t = x_pred_t + x_update
                x_filt_t = jnp.clip(x_filt_t, -MAX_STATE_VALUE, MAX_STATE_VALUE)
                IKC_obs = I_s - K @ C_obs_static
                P_filt_t = IKC_obs @ P_pred_t @ IKC_obs.T + K @ H_obs_static @ K.T
                P_filt_t = (P_filt_t + P_filt_t.T) / 2.0
                P_filt_t = P_filt_t + eps * I_s
            ...
            if n_obs_actual > 0:
                ...
                mahalanobis_dist_obs = 0.0
                try:
                    L_S_obs_ll = jnp.linalg.cholesky(S_obs_reg) # Re-cholesky for safety
                    ...
                    try:
                        ...
                        mahalanobis_dist_obs = 1e6 # Penalty
                
                ll_t = -0.5 * (log_pi_term + log_det_S_obs + mahalanobis_dist_obs)
                ll_t = jnp.where(sign > 0, ll_t, -1e6) # Ensure S_obs_reg was PD
            ll_t = jnp.where(jnp.isfinite(ll_t), ll_t, -1e6)

            outputs = { /* ... as before ... */ }
            # ... (sanitization as before) ...
            # --- (The sanitization block for outputs remains the same) ---
            for key_out in ['x_pred', 'x_filt', 'innovations']:
                if key_out in outputs and outputs[key_out].size > 0:
                    outputs[key_out] = jnp.where(jnp.isfinite(outputs[key_out]), outputs[key_out], jnp.zeros_like(outputs[key_out]))
            for key_out in ['P_pred', 'P_filt', 'innovation_cov']:
                if key_out in outputs and outputs[key_out].size > 0:
                    shape = outputs[key_out].shape
                    if len(shape) == 2 and shape[0] == shape[1]:
                        dim = shape[0]
                        outputs[key_out] = jnp.where(jnp.isfinite(outputs[key_out]), outputs[key_out], jnp.eye(dim, dtype=outputs[key_out].dtype) * eps)
            
            x_filt_t_safe = jnp.where(jnp.isfinite(x_filt_t), x_filt_t, jnp.zeros_like(x_filt_t))
            P_filt_t_safe = jnp.where(jnp.isfinite(P_filt_t), P_filt_t, jnp.eye(self.n_state, dtype=P_filt_t.dtype) * eps)

            return (x_filt_t_safe, P_filt_t_safe), outputs

        init_carry = (self.init_x, self.init_P)
        ys_reshaped = jnp.reshape(ys_arr, (-1, self.n_obs)) # This is fine
        (_, _), scan_outputs = lax.scan(step_static_nan, init_carry, ys_reshaped)
        # ... (final sanitization as before) ...
        for key_final in scan_outputs:
            if key_final == 'log_likelihood_contributions':
                scan_outputs[key_final] = jnp.where(jnp.isfinite(scan_outputs[key_final]), scan_outputs[key_final], jnp.full_like(scan_outputs[key_final], -1e6))
            elif scan_outputs[key_final].size > 0:
                scan_outputs[key_final] = jnp.where(jnp.isfinite(scan_outputs[key_final]), scan_outputs[key_final], jnp.zeros_like(scan_outputs[key_final]))
        return scan_outputs
    ...
    def _simulation_smoother_single_draw(self, ys: jax.Array, key: jax.random.PRNGKey, filter_results_orig: Optional[Dict] = None) -> jax.Array:
        ...
        Handles NaN values in the original data correctly, but simulated data has no NaNs.
        """
        Tsteps = ys.shape[0]
        if Tsteps == 0: 
            return jnp.empty((0, self.n_state), dtype=self.init_x.dtype)
        ...
        # IMPORTANT: Find which observation variables are always observed, 
        # for simulated data we only generate values for those positions
        try:
            # Get valid observation indices from first time step
            valid_obs_idx = jnp.where(~jnp.isnan(ys[0]))[0]
            
            # In case all rows have the same NaN pattern, extract only valid indices
            # for C matrix to generate only observable values
            C_valid = self.C[valid_obs_idx, :]
            H_valid = self.H[jnp.ix_(valid_obs_idx, valid_obs_idx)]
            
            # Generate noise only for observable variables
            eta_star_valid = self.simulate_obs_noise(key_eta, (Tsteps,))
            
            # Generate observations only for observable variables
            y_star_valid = jnp.zeros((Tsteps, self.n_obs), dtype=x_star.dtype)
            y_star_valid_only = (x_star @ C_valid.T) + eta_star_valid
            
            # Place the valid observations back into a full matrix with NaNs
            # matching the pattern of the original data
            y_star = jnp.full((Tsteps, self.n_obs), jnp.nan, dtype=y_star_valid_only.dtype)
            y_star = y_star.at[:, valid_obs_idx].set(y_star_valid_only)
            
        except (IndexError, ValueError):
            # Fallback: if no valid obs or error occurred, just create fully valid simulated data
    ...
    def simulation_smoother(self, ys: ArrayLike, key: jax.random.PRNGKey, num_draws: int = 1,
        ...
        """ 
        Runs the Durbin-Koopman simulation smoother with NaN handling.
        If the original data contains NaNs (missing values), the simulated data
        will have the same pattern of missingness.
        
        Args:
            ys: Observations array, shape `[T, n_obs]`. NaN indicates missing.
            key: JAX PRNG key for random draws.
            num_draws: Number of simulation smoother draws.
            filter_results: (Optional) Pre-computed filter results.
            
        Returns:
            If num_draws=1:
                jax.Array: State draws from the posterior [T, n_state]
            Else:
                Tuple of:
                    - mean_smooth_sim: Mean of posterior draws [T, n_state]
                    - median_smooth_sim: Median of posterior draws [T, n_state]
                    - all_draws_jax: All posterior draws [num_draws, T, n_state]
        """
        if num_draws <= 0: 
        ...
        if Tsteps == 0:
            if num_draws == 1: 
                ...
                return empty_state, empty_state, jnp.empty((num_draws, 0, self.n_state), dtype=self.init_x.dtype)
        ...
        if num_draws == 1:
            ...
            def perform_single_dk_draw(key_single_draw):
                # This reuses the x_smooth_rts_common for all draws
                return self._simulation_smoother_single_draw(
                    ys_arr, key_single_draw, 
                    filter_results_orig={'x_smooth_rts': x_smooth_rts_common}  # Pass directly
                )
            ...
            # Compute statistics across draws
