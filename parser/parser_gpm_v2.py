...
class ModelSolver:
    ...
    def create_simdkalman_filter(self):
        """Create simdkalman.KalmanFilter object."""
        self.kf = simdkalman.KalmanFilter(
            state_transition = self.ss["transition"],
            process_noise = self.ss["selection"],
            observation_model = self.ss["design"],
            observation_noise =  np.eye(len(self.labels['observable_labels']))*1e-5 #self.ss["D"]  # Remember to stabilize this value
        )
