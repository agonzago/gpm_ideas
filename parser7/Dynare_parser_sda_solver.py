...
def parse_lambdify_and_order_model(model_string, verbose=True):
    ...
    # --- Step 1d: Combine parameter NAMES --- <<< REVISED CHECK ---
    # Start with unique list of explicitly declared parameters
    combined_param_names = list(dict.fromkeys(param_names_declared).keys())
    # Add inferred sigma_ parameters if they weren't already in the combined list
    added_sigmas = []
    for p_sigma in inferred_sigma_params:
        if p_sigma not in combined_param_names:
            # print(f"   Adding inferred parameter for stationary shock std dev: {p_sigma}") # Keep for debugging
            combined_param_names.append(p_sigma)
            added_sigmas.append(p_sigma)
    if added_sigmas:
         print(f"   Added inferred stationary sigma parameters: {added_sigmas}")


    # --- Step 1e: Combine parameter ASSIGNMENTS ---
    # Start with defaults from stderr values parsed from 'shocks' command
    combined_param_assignments = sigma_stderr_assignments.copy()
    # Update/overwrite with explicit assignments from 'parameters' block or direct assignments
    combined_param_assignments.update(param_assignments_initial)
    # Ensure defaults exist for added sigma params if not set by stderr/explicitly
    for p_sigma in added_sigmas:
        if p_sigma not in combined_param_assignments:
             print(f"Warning: Inferred sigma parameter '{p_sigma}' has no default value from 'shocks' command or explicit assignment. Setting default to 1.0.")
             combined_param_assignments[p_sigma] = 1.0


    # --- Use the combined lists from now on ---
    param_names = combined_param_names # Use this final list
